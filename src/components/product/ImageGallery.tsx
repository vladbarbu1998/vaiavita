...
export const ImageGallery = ({ images, productName }: ImageGalleryProps) => {
  ...
  return (
    <>
      <div className="flex gap-4 opacity-0 animate-fade-up items-start">
        {/* Thumbnails - Left Side */}
        {images.length > 1 && (
          <div className="hidden md:flex flex-col w-20 shrink-0 gap-2">
            {/* Up Arrow */}
            {showThumbnailNavigation && (
              <Button
                variant="ghost"
                size="sm"
                className={`w-full h-7 ${!canScrollThumbnailsUp ? 'opacity-30 cursor-not-allowed' : ''}`}
              ...
              {visibleThumbnails.map((img, index) => {
                ...
                return (
                  <button
                    ...
                    className={`relative overflow-hidden shrink-0 aspect-square rounded-xl border-2 transition-all duration-200 bg-muted/30 ${
            ...
            {showThumbnailNavigation && (
              <Button
                variant="ghost"
                size="sm"
                className={`w-full h-7 ${!canScrollThumbnailsDown ? 'opacity-30 cursor-not-allowed' : ''}`}
        ...
        <div className="flex-1 relative min-w-0">
          ...
          <div 
            className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-muted/30 to-muted/10 cursor-zoom-in group w-full aspect-square touch-pan-y"
          ...
          >
            <img 
              ...
              alt={productName} 
              className="w-full h-full object-contain p-4 md:p-6 transition-transform duration-300 group-hover:scale-105 pointer-events-none select-none" 
              draggable={false}
            />
